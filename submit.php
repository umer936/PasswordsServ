...
<fieldset>
  <legend>Password Complexity Requirement</legend>
    ...
    <div class="input-field col s6">
      ...
      <label for="Spaces">Allows Spaces</label>
    </div>
    </div>
</fieldset>
    <fieldset>
    ...
    <div class="row">
      <div class="input-field col s6">
        <input id="HasMin" name="HasMin" type="checkbox" onchange="document.getElementById('Min').disabled = !this.checked; document.getElementById('MinMaxSlider').style.display = 'block'; if ( !this.checked ) {
    document.getElementsByClassName('noUi-origin')[0].setAttribute('disabled', true);
  } else {
    document.getElementsByClassName('noUi-origin')[0].removeAttribute('disabled');
  }" class="validate">
        <label for="HasMin">Has Min Length</label>
      </div>
      <div class="input-field col s6">
        <input id="HasMax" name="HasMax" type="checkbox" onchange="document.getElementById('Max').disabled = !this.checked; document.getElementById('MinMaxSlider').style.display = 'block';  if ( !this.checked ) {
    document.getElementsByClassName('noUi-origin')[1].setAttribute('disabled', true);
  } else {
    document.getElementsByClassName('noUi-origin')[1].removeAttribute('disabled');
  }" class="validate">
